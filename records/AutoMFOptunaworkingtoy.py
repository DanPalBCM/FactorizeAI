...
def objective(trial):
    ...
    if MF_name == "PCA_sklearn": # Connected with if statement as we did before
        ...
        X, X_r = PCA_sklearn(X_scaled, n_components=n_c)

    if MF_name == "NMF_sklearn":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = NMF_sklearn(X_scaled, n_components=n_c)

    if MF_name == "MiniBatch_NMF":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = MiniBatch_NMF(X_scaled, n_components=n_c)

    if MF_name ==  "Sparse_PCA":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Sparse_PCA(X_scaled, n_components=n_c)

    if MF_name == "Kernel_PCA":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Kernel_PCA(X_scaled, n_components=n_c)

    if MF_name == "Truncated_SVD":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Truncated_SVD(X_scaled, n_components=n_c)

    if MF_name == "Incremental_PCA":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Incremental_PCA(X_scaled, n_components=n_c)

    if MF_name == "IndependentCA":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = IndependentCA(X_scaled, n_components=n_c)

    if MF_name == "MiniBatch_SparsePCA":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = MiniBatch_SparsePCA(X_scaled, n_components=n_c)

    if MF_name == "Factor_Analysis":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Factor_Analysis(X_scaled, n_components=n_c)

    if MF_name == "Dictionary":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Dictionary(X_scaled, n_components=n_c)

    if MF_name == "LDA":
        n_c = trial.suggest_int("n_components", 2, 15)
        X, X_r = LDA(X_scaled, n_components=n_c)

    if MF_name == "t_SNE":
        n_c = trial.suggest_int("n_components", 2,3)
        X, X_r = t_SNE(X_scaled, n_components=n_c)

    if MF_name == "Iso_map":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Iso_map(X_scaled, n_components=n_c)

    if MF_name == "Local_Linear_Embedding":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Local_Linear_Embedding(X_scaled, n_components=n_c) 

    if MF_name == "Multidimensional_scaling":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Multidimensional_scaling(X_scaled, n_components=n_c)

    if MF_name == "Spectral_Embedding":
        n_c = trial.suggest_int("n_components", 2, min(X_scaled.shape[0], X_scaled.shape[1]))
        X, X_r = Spectral_Embedding(X_scaled, n_components=n_c)

    if MF_name == "U_MAP":    
        n_c = trial.suggest_int("n_components", 2,3)
        X, X_r = U_MAP(X_scaled, n_components=n_c)
