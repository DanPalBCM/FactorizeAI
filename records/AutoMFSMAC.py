...
#######################
# Matrix Factorization algorithms OLD
#######################

# NMF

def oNMF_sklearn(X, **kwargs):
...
def oMiniBatch_SparsePCA(X, **kwargs):
...
def oLocal_Linear_Embedding(X, **kwargs):
    transformer = LocallyLinearEmbedding(eigen_solver= 'auto', **kwargs)
    X_hat = transformer.fit_transform(X)

    return X_hat, X_hat

def oMultidimensional_scaling(X, **kwargs):
    transformer = MDS(normalized_stress='auto', **kwargs)
    X_hat = transformer.fit_transform(X)

    return X_hat, X_hat

def oSpectral_Embedding(X, **kwargs):
...
def Model_optimization(X, clusters, number_clusters,  optimal_model, mode, evaluation_method, visualizations):
    ...
    if optimal_model == "PCA_sklearn":
        ...
        svd_solver_list = ['auto', 'full', 'randomized'] # removing arpack due to errors
    ...
    if optimal_model == "Spectral_Embedding":
        ...
        param2_list = ['lobpcg', 'amg'] # omitting arpack due to errors 
...
## Reconstruction ##
    ...
    AutoMF_main(X, clusters, 2, mode = "Reconstruction", evaluation_method = "MSE",
            visualizations = False, n_components = 2, random_state = 42)
...
class NMF_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, init])
...
class KernelPCA_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, kernel])
...
class TruncatedSVD_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        cs = ConfigurationSpace(seed=0)
        power_iteration_normalizer = Categorical("power_iteration_normalizer", ['OR', 'LU'], default='OR')
        n_components = Integer("n_components", (2, 5), default=3)

        
        cs.add_hyperparameters([n_components, power_iteration_normalizer])
...
class IncrementalPCA_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        cs = ConfigurationSpace(seed=0)
        batch_size = Integer("batch_size", (10,20), default=10) # check if default None if it works, None = 5 * n_features we can use this instead (5*n, 5*n + 20)
        n_components = Integer("n_components", (2, 5), default=3)
...
class FastICA_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, fun])
...
class SparsePCA_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, method])
...
class FactorAnalysis_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, svd_method])
...
class DictionaryLearning_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, transform_algorithm])
...
class LatentDirichletAllocation_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, learning_method])
...
class Isomap_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        cs = ConfigurationSpace(seed=0)
        n_neighbors = Integer("n_neighbors", (5,15), default=10)
        eigen_solver = Categorical("eigen_solver", ['auto', 'arpack', 'dense'], default='auto')
        
        cs.add_hyperparameters([eigen_solver, n_neighbors])
...
class MDS_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        n_components = Integer("n_components", (2, 5), default=3)
        # add n_neigh = 25
        cs.add_hyperparameters([n_components, dissimilarity])
...
class SpectralEmbedding_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, eigen_solver])
...
class UMAP_BO:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters([n_components, n_neighbors])
...
def Decision_making():
    ...
    if TRAIN:
        ...
        X_train = []
        directory = "/mnt/belinda_local/daniel/home/AutoMF/NMFdir"
        ...
        # X_train family of matrices for training

        # Generate true output

        for i in range(len(X_train)):
            clusters = [0]
            AutoMF_main(X_train[i], clusters, 2, mode = "Reconstruction", evaluation_method = "MSE",
                visualizations = False, n_components = 2, random_state = 42)
              
            # Here we just assign y and then append to y_train

        y_train = []# Family of desired results, for now take most optimal method. string ** 
     # Save model

        # Classification global ML model, takes matrix, decides what method should be optimal based on training 
    PREDICT = False
    if PREDICT:
       X = [] # single matrix input
       y = [] # single string output with optimal method 
