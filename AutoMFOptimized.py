...
def objective(trial):
    ...
    if MF_name == "NMF_sklearn":
        ...
        X, X_r = NMF_sklearn(X_scaled, n_components=n_c, init=init, solver=solver, beta_loss=beta_loss, verbose=verbose, l1_ratio=l1_ratio, alpha_W=alpha_W)
    ...
    if MF_name == "MiniBatch_NMF":
        ...
        l1_ratio = trial.suggest_float("l1_ratio", 0.0, 0.9)
        alpha_W = trial.suggest_float("alpha_W", 0.0, 0.9)
        X, X_r = MiniBatch_NMF(X_scaled, n_components=n_c, init=init, beta_loss=beta_loss, batch_size=batch_size, l1_ratio=l1_ratio, alpha_W=alpha_W)
    ...
    if MF_name ==  "Sparse_PCA":
        ...
        X, X_r = Sparse_PCA(X_scaled, n_components=n_c, alpha=alpha, ridge_alpha=ridge_alpha)
    ...
    if MF_name == "Kernel_PCA":
        ...
        alpha = trial.suggest_float("alpha", 0.1, 1.0)
        X, X_r = Kernel_PCA(X_scaled, n_components=n_c, kernel=kernel, degree=degree, eigen_solver=eigen_solver, alpha=alpha)
    ...
    if MF_name == "Truncated_SVD":
        ...
        X, X_r = Truncated_SVD(X_scaled, n_components=n_c, n_iter=n_iter, n_oversamples=n_oversamples)
    ...
    if MF_name == "IndependentCA":
        ...
        X, X_r = IndependentCA(X_scaled, n_components=n_c, fun=fun, algorithm=algorithm, whiten_solver=whiten_solver)
    ...
    if MF_name == "MiniBatch_SparsePCA":
        ...
        X, X_r = MiniBatch_SparsePCA(X_scaled, n_components=n_c, alpha=alpha, ridge_alpha=ridge_alpha, batch_size=batch_size)
    ...
    if MF_name == "Dictionary":
        ...
        X, X_r = Dictionary(X_scaled, n_components=n_c, transform_algorithm=transform_algorithm, alpha=alpha, fit_algorithm=fit_algorithm)
    ...
    if MF_name == "LDA":
        ...
        X, X_r = LDA(X_scaled, n_components=n_c, learning_method=learning_method, learning_decay=learning_decay, learning_offset=learning_offset, max_iter=max_iter)
    ...
    if MF_name == "t_SNE":
        ...
        X, X_r = t_SNE(X_scaled, n_components=n_c, perplexity=perplexity, early_exaggeration=early_exaggeration, learning_rate=learning_rate, n_iter=n_iter, n_iter_without_progress=n_iter_without_progress, init=init, angle=angle)
    ...
    if MF_name == "Iso_map":
        ...
        X, X_r = Iso_map(X_scaled, n_components=n_c, n_neighbors=n_neighbors, path_method=path_method, neighbors_algorithm=neighbors_algorithm)
    ...
    if MF_name == "Local_Linear_Embedding":
        ...
        neighbors_algorithm = trial.suggest_categorical("neighbors_algorithm", ['auto', 'brute', 'kd_tree', 'ball_tree'])
        X, X_r = Local_Linear_Embedding(X_scaled, n_components=n_c, n_neighbors=n_neighbors, neighbors_algorithm=neighbors_algorithm)
    ...
    if MF_name == "Spectral_Embedding":
        ...
        X, X_r = Spectral_Embedding(X_scaled, n_components=n_c, eigen_solver=eigen_solver, affinity=affinity)
    ...
    if MF_name == "U_MAP":    
        ...
        X, X_r = U_MAP(X_scaled, n_components=n_c, n_neighbors=n_neighbors, min_dist=min_dist, metric=metric)
...
if Preliminary_runs == True:
    ...
    Model_auto_select(X_scaled, clusters, number_clusters, mode, evaluation_method, visualizations = False, name = "NMF", autoML_engine = "SMAC3", random_state = 0)
...
if testing_optuna_BO == True:
    ...
    # optimal_model = Model_auto_select(X_scaled, clusters, number_clusters, mode, evaluation_method, visualizations = False, name = "test", autoML_engine= "SMAC3", random_state = 0)
